...
const DriverRegistration = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post('http://localhost:5191/api/drivers/register', formData);
            ...
            {step === 1 && (
              <>
                ...
                <label className={labelStyle}>Phone & Emergency Contact</label>
                  <div className="flex gap-2"><div className="relative flex-1"><Phone className="absolute left-3 top-3.5 text-gray-400" size={18} /><input name="phone" value={formData.phone} onChange={handleInputChange} className={inputStyle} placeholder="Phone" /></div>
                  <div className="relative flex-1"><AlertTriangle className="absolute left-3 top-3.5 text-gray-400" size={18} /><input name="emergencyPhone" value={formData.emergencyPhone} onChange={handleInputChange} className={inputStyle} placeholder="Emergency #" /></div></div>
