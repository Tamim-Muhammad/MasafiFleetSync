...
const DriverAssignmentDetails = () => {
  ...
  const [isTelemetryActive, setIsTelemetryActive] = useState(true);
  ...
  useEffect(() => {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            try {
              // Background HTTP POST to server endpoint updating live coordinates
              /*
              await fetch('http://localhost:5191/api/driver/location', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ driverId: 'DR-10234', jobId: 'J-250518-01', lat: currentLat, lng: currentLng })
              });
              */
    ...
    sendTelemetry();
    const interval = setInterval(sendTelemetry, 30000); // exactly 30s interval as specified
